  ...
  return (
    <div className={cn("flex flex-col bg-background", fullscreen ? "h-screen" : "h-full border-l border-border")}>
      <div className={cn("flex items-center gap-2 px-3 py-2 border-b border-border shrink-0 flex-wrap", fullscreen && "bg-background/90 backdrop-blur-sm")}>
        ...
        {versions.length >= 1 && (
          <div className="flex min-w-0 items-center gap-1.5 text-xs text-muted-foreground">
            ...
            <VersionChips
              ...
              className="max-w-[min(220px,28vw)] overflow-x-auto"
            />
            <button
              type="button"
              onClick={() =>
              ...
              <>
                ...
                <button
                  ...
                  className={cn(
                    ...
                    isCompareMode
                      ? "border border-amber-500/20 bg-amber-500/20 text-amber-500 shadow-[0_0_15px_rgba(245,158,11,0.1)]"
                      : "text-muted-foreground hover:bg-accent hover:text-foreground disabled:opacity-30"
                  )}
                  title="Compare with previous version"
                >
                  <Split className="h-3 w-3" />
                  <span className="hidden sm:inline">UI Duel</span>
                </button>
              </>
            )}
          </div>
       ...
       {activeVersion && (
        <div className="flex shrink-0 items-center justify-between gap-2 border-b border-border bg-card/80 px-3 py-1.5 sm:px-4">
          <div className="flex min-w-0 items-center gap-2">
            ...
            />
            <span className="shrink-0 font-mono text-[11px] font-semibold tabular-nums text-foreground/90">
