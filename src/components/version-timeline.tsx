...
import { checkpointProgressTitle } from "@/lib/file-checkpoint";
...
function partialLabel(version: CodeVersion | undefined): string | null {
  if (!version?.code) return null;
  return checkpointProgressTitle(version.code);
}
    ...
    <div className="flex max-w-[min(96vw,640px)] flex-col gap-1.5 rounded-2xl border border-white/10 bg-black/55 px-3 py-2 shadow-2xl backdrop-blur-md">
      ...
      <div className="flex min-w-0 flex-1 items-center gap-1 overflow-x-auto scrollbar-thin">
        {indices.map((idx, i) => {
          ...
          const partial = partialLabel(versions[idx]);
          return (
            <div key={idx} className="flex shrink-0 items-center gap-1">
              ...
              <button
                type="button"
                title={`${partial ? `${partial}. ` : ""}${title}${latest ? " (latest)" : ""}`}
                onClick={() => onVersionChange(idx)}
                className={cn(
                  "rounded-full px-2 py-0.5 font-mono text-[11px] font-semibold tabular-nums transition-all",
                  partial
                    ? active
                      ? "bg-amber-400/25 text-amber-100 shadow-[0_0_12px_rgba(251,191,36,0.2)] ring-1 ring-amber-300/50"
                      : "bg-amber-400/10 text-amber-200/80 hover:bg-amber-400/20"
                    : active
      ...
      <div className="flex max-w-[11.5rem] shrink-0 flex-col items-end border-l border-white/10 pl-2.5">
        <span className="font-mono text-[10px] leading-none text-white/90">
          ...
          {partialLabel(versions[activeVersionIndex]) ? (
            <span className="ml-1 text-[8px] font-bold uppercase tracking-wide text-amber-200/90">
              cut
            </span>
          ) : isLatest ? (
        ...
        <span
          className={cn(
            "mt-0.5 line-clamp-2 text-right text-[9px] leading-tight",
            partialLabel(versions[activeVersionIndex]) ? "text-amber-100/90" : "text-white/40"
          )}
        >
          {partialLabel(versions[activeVersionIndex]) ||
            versions[activeVersionIndex]?.title?.slice(0, 22) ||
            "Checkpoint"}
        </span>
      </div>
      </div>
