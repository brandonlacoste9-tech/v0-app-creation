...
export const DESIGN_STYLES: DesignStyle[] = [
  ...
  },
  {
    ...
    typography:
      "TYPE SCALE — closed. Only these text-* utilities may appear, each bound to one role: text-7xl md:text-9xl font-black uppercase tracking-[-0.07em] leading-[0.78] is the H1 AND the one editorial sentence only, one line, ending with an #E24A2A period. text-3xl font-black tabular-nums tracking-[-0.04em] is the 01/02/03 object index only. text-2xl font-black tracking-[-0.06em] tabular-nums is the price only (via formatMoney, never text-sm, never raw cents) and the header/footer wordmark. text-sm is the card name (font-black uppercase tracking-tight) and body copy only. text-[11px] uppercase tracking-[0.22em] font-black is the announcement, nav, collection label, button labels, quick-add, trust strip, and footer links. Any other text-* class — text-lg, text-xl, text-4xl, text-5xl, text-6xl, text-base, or anything else — fails the check. This closed five-role scale supersedes any looser scale guidance.",
    effects:
    ...
    avoid:
      "Generic 3-column grids, centered heroes, timid text-3xl heroes, any text size outside the closed five-role scale, rounded-2xl, rounded-lg or shadow-md on product cards, pill buttons on cards, glassmorphism, pastel wellness, lorem, invented products, invented stills, mixing inline SVG and raster stills in one grid, per-product illustration styles, text-center or mx-auto on the header wordmark / hero H1 / collection label / grid / footer wordmark, hero carousels, second images or button rows beside the H1, hero video, clip-art icon salad, section padding other than py-16 md:py-24, grid gaps other than gap-6 md:gap-10, inputs without an associated label, focus that is not visible",
